function Show-Pixel {
  ...
  .EXAMPLE
  $SecretRecipe = Show-Pixel -sf "$env:temp\stego.png" -cc RGB -bpc 1
  $SecretRecipe | UnProtect-MySecret -Passphrase (ConvertTo-SecureString "Good Boy, Duke!" -AsPlainText -Force)

  .EXAMPLE
  Show-Pixel -sf "$home\Documents\stego.png" -cc RGB -bpc 2 | Out-File "$home\Documents\stego.enc"
  ...
  .EXAMPLE
  Show-Pixel -sf "$home\Documents\payload.png" -cc RGB -bpc 2 | Out-File "$home\Documents\payload.enc"
  $cert = Get-Item Cert:\CurrentUser\My\833ED9148FD08F577D2AD743BAF71295AFEF345C
