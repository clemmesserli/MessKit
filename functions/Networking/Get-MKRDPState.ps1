function Get-MKRDPState {
  ...
  .EXAMPLE
  Get-MKRDPState -Credential $Credential -ComputerName L1PC1001
  Retrieve RDP info for a single computer

  .EXAMPLE
  Get-MKRDPState -Credential $Credential -ComputerName L1PC1001, L2PC1101
  Retrieve RDP info for a multiple computers

  .EXAMPLE
  Get-MKRDPState -Credential $Credential -ComputerName (Get-RDPLog -StartTime (Get-Date).AddDays(-7)) -ThrottleLimt 12
