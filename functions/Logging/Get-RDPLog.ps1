function Get-RDPLog {
  ...
  .EXAMPLE
  Get-RDPLog -StartTime (Get-Date).AddDays(-14)
  List unique RDP connections logged from the start date through the current datetime.

  .EXAMPLE
  Get-RDPLog -StartTime "01/01/2024" -EndTime "06/23/2024"
  List unique RDP connections logged between Midnight to Midnight for the dates provided.

  .EXAMPLE
  Get-RDPLog -StartTime (Get-Date -Year (Get-Date).Year -Month 6 -Day 1) -EndTime (Get-Date -Year (Get-Date).Year -Month 7 -Day 1)
  List unique RDP connections logged between June 01 - July 01 of current year.

  .EXAMPLE
  Get-RDPLog -StartTime (Get-Date).Date -EndTime (Get-Date).Date.AddHours(8)
